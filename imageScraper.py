...
while k < 10:
    ...
    print("NOW SCRAPING PAGE %s" % k)
    frontPage = threadCatalog[k]['threads']

    #Create a file to list all threads we've analyzed
    allThreadFile = saveDirectory + boardLetter + "/Page " + str(k) + str(datetime.datetime.now()) + ".csv"
    ...
    #There are always 15 threads on the front page. So we'll loop through and read each one
    i = 0
	...
	# First we create a new directory for the thread
	    ...
	    for post in individualThread['posts']:
	    	...
	    	with open(path + "/thread.csv", mode='a') as thread_file:
	    		thread_writer = csv.writer(thread_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_ALL)
	    		thread_writer.writerow([timeStamp, posterID, postID, subjectText, commentText, filename])
